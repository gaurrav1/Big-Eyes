...
export const Location = () => {
  ...
  const checkCitiesExceedingDistance = useCallback((newCenter) => {
  ...
  const handleConfirmCenterChange = useCallback(() => {
    ...
    updateAppData({
      ...
      otherCities: appData.otherCities.filter(city => 
        !citiesExceedingDistance.some(exceedingCity => 
  ...
  }, [pendingCenterCity, updateAppData, appData.otherCities, citiesExceedingDistance]);
  ...
  const handleCenterSelect = useCallback((location) => {
      ...
      >
        <div className={styles.dialogCityList}>
          {citiesExceedingDistance.map((city, index) => {
            ...
            return (
              <div key={`${city.label}-${index}`} className={styles.dialogCityItem}>
